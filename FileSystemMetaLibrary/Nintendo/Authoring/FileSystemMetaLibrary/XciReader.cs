...
// Type: Nintendo.Authoring.FileSystemMetaLibrary.XciReader
...
{
  public class XciReader : Sha256PartitionFileSystemArchiveReader
  {
    public XciReader(Stream stream)
      : base(stream)
    {
      // ISSUE: fault handler
      try
    ...
    public unsafe XciPartitionReader OpenXciPartitionReader(string fileName)
    {
      ...
      XciPartitionReader xciPartitionReader;
      // ISSUE: fault handler
      try
      {
        ...
        // ISSUE: explicit reference operation
        ^(int&) ((IntPtr) &sharedPtrNnFsIstorage1 + 4) = 0;
        xciPartitionReader = new XciPartitionReader(&sharedPtrNnFsIstorage2);
        GC.KeepAlive((object) this);
      }
      __fault
      {
